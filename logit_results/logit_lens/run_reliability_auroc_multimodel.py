...
def load_image(image_input):
    """Load image from URL or return PIL image directly."""
...
def extract_features_paligemma(model, processor, samples, device='cuda'):
    ...
    for sample in tqdm(samples):
        image = load_image(sample.get('image_url') or sample.get('image'))
        if image is None:
            continue
        ...
        try:
            ...
            with torch.no_grad():
                ...
                is_correct = pred_text.startswith(gt) or gt.startswith(
                    pred_text) or pred_text in gt or gt in pred_text
...
def compute_aurocs(results, model_name):
    ...
    try:
        spatial_auroc = roc_auc_score(
            correctness,
            -entropies)  # Negative because lower entropy = more confident
    except:
        spatial_auroc = 0.5
...
def main():
    ...
    try:
        ...
        paligemma_features = extract_features_paligemma(
    ...
    for model_name, result in all_results.items():
        if result:
            print(f"\n{model_name.upper()}:")
            print(
                f"  Spatial Attention AUROC: {result['spatial_attention_auroc']:.3f}"
            )
            print(
                f"  Combined Model AUROC:    {result['combined_auroc_test']:.3f}"
            )
        else:
