...
class Qwen2VLLogitLensAnalyzer:
    ...
    def analyze_sample(self, image: Image.Image, question: str, ground_truth: str) -> LogitLensResult:
        ...
        predicted_token = self.processor.tokenizer.decode([predicted_id]).strip().lower()
...
class NeuronAnalyzer:
    ...
    def setup_hooks(self, target_layers: List[int] = None):
        ...
        for layer_idx in target_layers:
            ...
            hook = layer.mlp.register_forward_hook(self._hook_mlp_neurons(layer_idx))
    ...
    def analyze_sample(self, image: Image.Image, question: str, ground_truth: str) -> Dict:
...
def load_vqa_samples(n_samples: int = 200) -> List[Dict]:
    ...
    records_path = os.path.join(PARENT_DIR, "test_intervention_output/analysis_records.json")
...
def run_logit_lens_analysis(model_type: str, n_samples: int = 200):
    ...
    acc_with = n_correct_with / len(results_with_image) if results_with_image else 0
    ...
    margins_no = np.stack([r.layer_margins for r in results_no_image[:len(results_with_image)]])
    ...
    attn_contribs = np.stack([r.attn_contributions for r in results_with_image])
    ...
    correct_margins = np.stack([r.layer_margins for r in correct_results]) if correct_results else None
    incorrect_margins = np.stack([r.layer_margins for r in incorrect_results]) if incorrect_results else None
    ...
    results = {
        ...
        "acc_with_image": float(acc_with),
        "acc_no_image": float(acc_no),
        ...
        "peak_visual_layer": int(peak_layer),
        "peak_visual_effect": float(peak_effect),
        "avg_mlp_contributions": avg_mlp.tolist(),
        "avg_attn_contributions": avg_attn.tolist(),
        "total_mlp_contribution": float(avg_mlp.sum()),
        "total_attn_contribution": float(avg_attn.sum()),
        "mlp_percent": float(avg_mlp.sum() / (avg_mlp.sum() + avg_attn.sum()) * 100),
        "attn_percent": float(avg_attn.sum() / (avg_mlp.sum() + avg_attn.sum()) * 100),
    }
    ...
    # Save results
    output_path = os.path.join(SCRIPT_DIR, f"{model_type}_logit_lens_results.json")
...
def run_neuron_analysis(model_type: str, n_samples: int = 200):
    ...
    if model_type == "qwen2-vl":
        ...
        target_layers = [n_layers - 8, n_layers - 5, n_layers - 3, n_layers - 2, n_layers - 1]
        target_layers = [l for l in target_layers if l >= 0]
    ...
    for layer_idx in target_layers:
        if layer_idx not in all_activations or len(all_activations[layer_idx]) == 0:
            continue
        ...
        probe = LogisticRegression(penalty='l1', solver='saga', max_iter=1000, C=0.1)
    ...
    # Save
    output_path = os.path.join(SCRIPT_DIR, f"{model_type}_neuron_analysis_results.json")
    with open(output_path, 'w') as f:
...
def main():
    parser = argparse.ArgumentParser(description="Multi-model logit lens and neuron analysis")
    parser.add_argument("--model", type=str, required=True, choices=["qwen2-vl", "paligemma"],
                       help="Model to analyze")
    parser.add_argument("--analysis", type=str, default="both", choices=["logit_lens", "neuron", "both"],
