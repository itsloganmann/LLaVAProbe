...
class AnalysisRecord:
    ...
    per_head_topk_info: Optional[Dict[str, Any]] = None
    ...
    intervention_results: Optional[List[Dict[str, Union[float, str,
                                                        bool]]]] = None
...
class StructuredOutputs:
    ...
    def to_json(self) -> str:
        payload = {
            "records":
            [self._record_to_dict(record) for record in self.records],
            "calibration":
            [asdict(breakdown) for breakdown in self.calibration],
        }
    ...
    def _record_to_dict(record: AnalysisRecord) -> dict:
        ...
        return {
            ...
            record.head_delta,
            "per_head_topk_info":
            record.per_head_topk_info,
            "ground_truth_delta":
            record.ground_truth_delta,
            "ground_truth_tokens":
            list(record.ground_truth_tokens)
            if record.ground_truth_tokens else None,
            "ablations":
            record.ablations,
            "run_mode":
            record.run_mode,
...
class AnalysisWriter:
    ...
    def write_json(self,
                   ...
                   filename: str = "analysis_records.json") -> Path:
    ...
    def write_csv(self,
                  outputs: StructuredOutputs,
                  filename: str = "analysis_records.csv") -> Path:
        path = self.output_dir / filename
