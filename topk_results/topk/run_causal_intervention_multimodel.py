...
def get_patch_attention_scores(attentions,
                               num_image_patches,
                               model_type,
                               image_token_start=0):
    """
...
def create_masked_image(image, mask_indices, patch_size=14, grid_size=16):
    ...
    if image.size[0] != expected_size or image.size[1] != expected_size:
        image_resized = image.resize((expected_size, expected_size),
                                     Image.BILINEAR)
...
def load_image(image_url):
    ...
    try:
        ...
        image = Image.open(
            requests.get(image_url, stream=True, timeout=10).raw)
        if image.mode != "RGB":
...
def run_causal_intervention(model_name, n_samples=200):
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing"):
        ...
        try:
            ...
            attentions, full_answer = mechanism.get_attention_and_answer(
                image, question)
            ...
            high_indices = get_mask_indices(patch_attn[:actual_patches],
                                            actual_patches, MASK_RATIO, "high")
            high_masked_image = create_masked_image(image,
                                                    high_indices,
                                                    patch_size=14,
                                                    grid_size=grid_size)
            high_answer = mechanism.get_answer_for_masked_image(
                high_masked_image, question)
            high_correct = check_answer_match(high_answer, ground_truth)
            results["high"].append(high_correct)

            # 3. Mask LOW attention patches
            low_indices = get_mask_indices(patch_attn[:actual_patches],
                                           actual_patches, MASK_RATIO, "low")
            low_masked_image = create_masked_image(image,
                                                   low_indices,
                                                   patch_size=14,
                                                   grid_size=grid_size)
            low_answer = mechanism.get_answer_for_masked_image(
                low_masked_image, question)
            low_correct = check_answer_match(low_answer, ground_truth)
            results["low"].append(low_correct)

            # 4. Mask RANDOM patches
            random_indices = get_mask_indices(patch_attn[:actual_patches],
                                              actual_patches, MASK_RATIO,
                                              "random")
            random_masked_image = create_masked_image(image,
                                                      random_indices,
                                                      patch_size=14,
                                                      grid_size=grid_size)
            random_answer = mechanism.get_answer_for_masked_image(
                random_masked_image, question)
    ...
    print(
        f"  Mask HIGH 30%:      {high_acc:.2f}%  (Δ = {high_acc - full_acc:+.2f}%)"
    )
    print(
        f"  Mask LOW 30%:       {low_acc:.2f}%  (Δ = {low_acc - full_acc:+.2f}%)"
    )
    print(
        f"  Mask RANDOM 30%:    {random_acc:.2f}%  (Δ = {random_acc - full_acc:+.2f}%)"
    )
    ...
    summary = {
        ...
        bool(high_acc < low_acc and p_high_low < 0.05),
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-Model Causal Intervention Analysis")
