...
def get_bsvalues_qwen(vector, model):
    ...
    norm_weight = model.model.language_model.norm.weight.data.to(device).to(
        dtype)
...
def duplicate_points(datapoints, min_dup=1, max_dup=9):
    ...
    if values.max() == values.min():
        ...
        scaled = ((values - values.min()) / (values.max() - values.min()) *
                  (max_dup - min_dup) + min_dup + 1).astype(int)
    weighted_points = np.concatenate(
        [np.repeat([pt], rep, axis=0) for pt, rep in zip(datapoints, scaled)],
        axis=0)
...
def calculate_metrics(db, weighted_attentions):
    ...
    for cluster in unique_clusters:
        ...
        avg_strength = np.mean(cluster_points) if len(
            cluster_points) > 0 else 0.0
...
class Qwen2VLMechanism:
    ...
    def __init__(self, device="cuda"):
        ...
        print(
            f"  GQA config: {self.num_heads} heads, {self.num_kv_heads} KV heads, {self.num_kv_groups} groups"
        )
    ...
    def get_attention_patches(self, image, question, K=5):
        ...
        for layer_idx in range(num_layers):
            if layer_idx >= len(hidden_states) - 1 or layer_idx >= len(
                    attentions):
                continue
            ...
            V_heads = V_expanded.permute(1, 0,
                                         2)  # [num_heads, seq_len, head_dim]

            # Compute attention output per head
            attn_out = torch.bmm(attn_weights,
            ...
            o_weight_split = o_weight.view(hidden_size, self.num_heads,
            ...
            base_log_prob = torch.log(
                get_prob(base_logits)[predicted_idx] + 1e-10)
            ...
            for head_idx in range(self.num_heads):
                ...
                added_log_prob = torch.log(
                    get_prob(added_logits)[predicted_idx] + 1e-10)
        ...
        if len(top_k_heads) == 0:
            return normalize(np.zeros(576).tolist(
            )), token_confidence, self.processor.decode(predicted_idx)
        ...
        for layer_idx, head_indices in heads_by_layer.items():
            ...
            for head_idx in head_indices:
                ...
                attn_to_last = attn_weights[
                    head_idx,
                    -1, :]  # [seq_len] - attention from last token to all
                ...
                o_proj_this_head = o_weight_split[
                    head_idx]  # [head_dim, hidden]

                # Per-position contribution to hidden state
                per_pos_hidden = torch.mm(
                    V_this_head, o_proj_this_head)  # [seq_len, hidden]
                ...
                if len(per_pos_array) == len(aggregated_scores):
                    ...
                    aggregated_scores += w * per_pos_array[:len(
                        aggregated_scores)]
        ...
        return aggregated_norm, token_confidence, self.processor.decode(
            predicted_idx)
...
class PaliGemmaMechanism:
    ...
    def get_attention_patches(self, image, question, K=5):
        ...
        if len(top_k_heads) == 0:
            return normalize(np.zeros(256).tolist(
            )), token_confidence, self.processor.decode(predicted_idx)
...
def run_analysis(model_name, top_k=5, n_samples=100):
    ...
    for idx, item in enumerate(tqdm(data, desc="Processing")):
        try:
            ...
            if model_name != "qwen2-vl" and len(
                    aggregated_norm) == patch_size * patch_size:
                try:
                    scores_2d = np.array(aggregated_norm).reshape(
            ...
            if (idx + 1) % 50 == 0:
                ...
                print(
                    f"  Last entropy: {attention_entropy:.4f}, confidence: {token_confidence:.4f}"
                )
    ...
    for qt in df['question_type'].unique():
        ...
        if len(subset) >= 3:
            r, _ = pearsonr(subset['attention_entropy'],
                            subset['token_confidence'])
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-model Top-K attention analysis")
